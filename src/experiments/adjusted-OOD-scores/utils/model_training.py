...
def sum_xval_folds(model, model_results_folder, num_cv_folds=5, verbose=1, **kwargs):
    ...
    np.save(f'{model_results_folder}_{model}/in_train_dataset_class_labels.npy', labels)
    np.save(f'{model_results_folder}_{model}/in_train_dataset_pred_probs.npy', pred_probs)
    np.save(f'{model_results_folder}_{model}/in_train_dataset_images.npy', images)
